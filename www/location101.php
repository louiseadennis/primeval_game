...
<?php
    ...
    print_health($db);
    print_wait($db);
    $prev_location = get_value_from_users("prev_location", $db);
    
    if (!junction_here($db)) {
        create_anomaly_junction($db);
    }
    
    $coin = rand(0,1);
    if ($coin == 1) {
        $d10 = rand(1, 10);
        change_anomaly($d10, $prev_location, $db);
    }
    print "</div>";
...
<div class=location>
<img src=assets/location101.png>
<h2>An Anomaly Junction</h2>

<p>You are standing on a pebbly beach by a lake.  Anomalies stretch as far as the eye can see.  There will probably be a lot of random creatures wandering about here.  A large cross is marked out in stones.  As you stand at the cross:</p>
